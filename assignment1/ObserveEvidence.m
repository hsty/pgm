...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
			...
			indexes = find(assignments(:, indx) == x);
			newValues = GetValueOfAssignment(F(j), IndexToAssignment(indexes', F(j).card));
			newAssignments = IndexToAssignment(indexes', F(j).card);
			
			newFactor = F(j);
			newFactor.val = zeros(1, prod(F(j).card));
			F(j) = SetValueOfAssignment(newFactor, newAssignments, newValues);
